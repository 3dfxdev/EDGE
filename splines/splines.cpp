...
int FS_LoadFile( const char *qpath, void **buffer );
...
bool idCameraDef::getCameraInfo(long time, idVec3_t &origin, idVec3_t &direction, idVec3_t target, float *fv) {

	//if (pauseTime < time) time -= pauseTime;	//????????????????????????????????????

	if ((time - startTime) / 1000 > totalTime && !followingMode) return false;
	...
	if (followingMode) {
		idVec3_t o1 = *cameraPosition->getPosition(step), o2 = *cameraPosition->getPosition(step + stride);// direction problem...
		idVec3_t dir = o2 - o1;

		idVec3_t tdir = target - trackedPosition;
		dot = DotProduct(tdir, dir);
		step += dot * stride;
		trackedPosition = target;

		if (step < startTime) step = startTime;
		else if (step > (totalTime * 1000 * stride)) step = (totalTime * 1000 * stride);

		origin = *cameraPosition->getPosition(step);
		*fv = fov.getFOV(step);
	} else {
...
qboolean idCameraDef::load(const char *filename) {
	char *buf;
	const char *buf_p;
	int length = FS_LoadFile( filename, (void **)&buf );
	if ( !buf ) {
		return qfalse;
	}
...
idCameraTrigger::idCameraTrigger() {}
idCameraTrigger::idCameraTrigger(idTriggerDef &t) {}
idCameraTrigger::~idCameraTrigger() {}

void idCameraTrigger::parse(const char *(*text)  ) {
	const char *token;
	//Com_MatchToken( text, "{" );		///????????
	do {
		token = Com_Parse( text );
		...
		do {
			...
			if (Q_stricmp(key.c_str(), "triggerAt") == 0) {
				TriggerDistance = atof(token);
			} else if (Q_stricmp(key.c_str(), "releaseAt") == 0) {
				ReleaseDistance = atof(token);
			} else if (Q_stricmp(key.c_str(), "firstView") == 0) {
				FirstCam = atoi(token);
			} else if (Q_stricmp(key.c_str(), "secondView") == 0) {
				SecondCam = atoi(token);
			} else if (Q_stricmp(key.c_str(), "position") == 0) {
				Com_UngetToken();
				Com_Parse1DMatrix( text, 3, TriggerPosition );
			}
			token = Com_Parse(text);
...
qboolean idCameraTrigger::load(const char *filename) {
	char *buf;
	const char *buf_p;
	int length = FS_LoadFile( filename, (void **)&buf );
	if ( !buf ) {
...
idCameraTrigger trigger[MAX_CAMERAS];
int currentCam = 0, camCounter = 0, trigCounter = 0;
...
extern "C" {
	...
	qboolean loadCamera(int camNum, const char *name) {
		if (camNum >= MAX_CAMERAS) return qfalse;
		camera[camNum].clear();
		currentCam = camNum;
		camCounter++;
	...
	qboolean getCameraInfo(int camNum, int time, float *origin, float *angles, float *fov) {
		if (camNum >= camCounter || camNum >= MAX_CAMERAS) return qfalse;
		idVec3_t dir, org;
		org[0] = origin[0];
		org[1] = origin[1];
		org[2] = origin[2];
	...
	qboolean getCameraInfoTarget(int camNum, int time, float *target, float *origin, float *angles, float *fov) {
		if (!camera[camNum].isTracking()) return getCameraInfo(camNum, time, origin, angles, fov);
		if (camNum >= camCounter || camNum >= MAX_CAMERAS) return qfalse;
		idVec3_t dir, org, targ;
		org[0] = origin[0];
		org[1] = origin[1];
		org[2] = origin[2];
		targ[0] = target[0];
		targ[1] = target[1];
		targ[2] = target[2];
		if (camera[camNum].getCameraInfo(time, org, dir, targ, fov)) {
			origin[0] = org[0];
			origin[1] = org[1];
			origin[2] = org[2];
			angles[0] = -asin (dir[2])*180/3.14159;
	...
	void startCamera(int camNum, int time) {
		if (camNum >= camCounter || camNum >= MAX_CAMERAS) return;
		camera[camNum].startCamera(time);
		currentCam = camNum;
	}

	void setPausedTime(void) {
		for (int i = 0; i < camCounter; ++i) camera[i].setPausedTime(getCurrentRendererTime());
	}

	void togglePause(int camNum) {
		if (camNum >= camCounter || camNum >= MAX_CAMERAS) return;
		camera[camNum].togglePause();
	}

	void toggleTracking(int camNum) {
		if (camNum >= camCounter || camNum >= MAX_CAMERAS) return;
		camera[camNum].toggleTracking();
	}

	void toggleFollowing(int camNum) {
		if (camNum >= camCounter || camNum >= MAX_CAMERAS) return;
		camera[camNum].toggleFolowing();
	}
	...
	void saveCamera(int camNum, const char *name) {
		if (camNum >= camCounter || camNum >= MAX_CAMERAS) return;
		camera[camNum].save(name);
		currentCam = camNum;
	}

	void setCurrentCam(int camNum) {
		currentCam = camNum;
		if (!camera[camNum].isRunning()) camera[camNum].startCamera(getCurrentRendererTime());
	}

	qboolean loadCameraTrigger(int trigNum, const char *name) {
		if (trigNum >= MAX_CAMERAS) return qfalse;
		trigCounter++;
		return static_cast<qboolean>(trigger[trigNum].load(name));
	}

	void toggleCameraTrigger(int trigNum) {
		if (trigNum < trigCounter) trigger[trigNum].Used = !trigger[trigNum].Used;
	}

	void switchCameras(idTriggerDef *tg) {
		idCameraTrigger t = *((idCameraTrigger *)tg);
		// do camera switching...
		if (currentCam == t.FirstCam) setCurrentCam(t.SecondCam);
		else if (currentCam == t.SecondCam) setCurrentCam(t.FirstCam);
		// any other camera is omitted
	}

	int getCurrentCamera(float *target) {
		if (target) {
			idVec3_t p;
			p[0] = target[0];
			p[1] = target[1];
			...
			for (int i = 0; i < trigCounter; ++i) trigger[i].doAction(p, switchCameras);
		}
		return currentCam;
	}
}
