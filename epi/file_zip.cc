...
static bool load_end_part()
{
	...
	// fix endianness
	r_end_part.this_disk        = EPI_LE_U16(r_end_part.this_disk);
	r_end_part.central_dir_disk = EPI_LE_U16(r_end_part.central_dir_disk);
	r_end_part.disk_entries     = EPI_LE_U16(r_end_part.disk_entries);
	r_end_part.total_entries    = EPI_LE_U16(r_end_part.total_entries);
	r_end_part.comment_length   = EPI_LE_U16(r_end_part.comment_length);

	r_end_part.dir_size   = EPI_LE_U32(r_end_part.dir_size);
	r_end_part.dir_offset = EPI_LE_U32(r_end_part.dir_offset);
...
static bool read_directory_entry(zip_central_entry_t *E)
{
	...
	// fix endianness
	E->hdr.made_version    = EPI_LE_U16(E->hdr.made_version);
	E->hdr.req_version     = EPI_LE_U16(E->hdr.req_version);

	E->hdr.flags           = EPI_LE_U16(E->hdr.flags);
	E->hdr.comp_method     = EPI_LE_U16(E->hdr.comp_method);
	E->hdr.file_time       = EPI_LE_U16(E->hdr.file_time);
	E->hdr.file_date       = EPI_LE_U16(E->hdr.file_date);

	E->hdr.crc             = EPI_LE_U32(E->hdr.crc);
	E->hdr.compress_size   = EPI_LE_U32(E->hdr.compress_size);
	E->hdr.full_size       = EPI_LE_U32(E->hdr.full_size);

	E->hdr.name_length     = EPI_LE_U16(E->hdr.name_length);
	E->hdr.extra_length    = EPI_LE_U16(E->hdr.extra_length);
	E->hdr.comment_length  = EPI_LE_U16(E->hdr.comment_length);
	E->hdr.start_disk      = EPI_LE_U16(E->hdr.start_disk);

	E->hdr.internal_attrib = EPI_LE_U16(E->hdr.internal_attrib);
	E->hdr.external_attrib = EPI_LE_U32(E->hdr.external_attrib);

	E->hdr.local_offset    = EPI_LE_U32(E->hdr.local_offset);
...
void ZIPF_CloseWrite(void)
{
	...
	end_part.disk_entries  = EPI_LE_U16(total_entries);
	end_part.total_entries = EPI_LE_U16(total_entries);

	end_part.dir_offset = EPI_LE_U32(dir_offset);
	end_part.dir_size   = EPI_LE_U32(dir_size);
...
void ZIPF_NewLump(const char *name)
{
	...
	w_local.hdr.req_version = EPI_LE_U16(ZIPF_REQ_VERSION);
	w_local.hdr.flags = 0;

	// no compression... yet...
	w_local.hdr.comp_method = EPI_LE_U16(ZIPF_COMP_STORE);

	w_local.hdr.file_date = EPI_LE_U16(zipf_date);
	w_local.hdr.file_time = EPI_LE_U16(zipf_time);
	...
	w_local.hdr.name_length  = EPI_LE_U16(name_length);
...
void ZIPF_FinishLump(void)
{
	...
	w_local.hdr.full_size     = EPI_LE_U32(w_local_length);
	w_local.hdr.compress_size = EPI_LE_U32(w_local_length);
	...
	central.hdr.made_version = EPI_LE_U16(ZIPF_MADE_VERSION);
	...
	central.hdr.external_attrib = EPI_LE_U32(ZIPF_ATTRIB_NORMAL);

	central.hdr.local_offset = EPI_LE_U32(w_local_start);
