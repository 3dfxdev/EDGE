...
//
UI_ClientList::UI_ClientList(int x, int y, int w, int h) :
...
//
UI_ClientList::~UI_ClientList()
{
}

//------------------------------------------------------------------------

int UI_ClientList::get_column_width(int col, int total_W) const
{
	SYS_ASSERT(col >= 0);
...
const char * UI_ClientList::get_column_name(int col) const
{
	SYS_ASSERT(col >= 0);
...
void UI_ClientList::resize(int x, int y, int w, int h)
{
...
void UI_ClientList::draw_cell(TableContext context, int R, int C,
    ...
    switch (context)
    {
		...
		case CONTEXT_CELL:
		{
			fl_push_clip(X, Y, W, H);
			{
				...
				// TEXT
				fl_color(FL_BLACK);
				fl_draw("Cell", X, Y, W, H, FL_ALIGN_CENTER);
