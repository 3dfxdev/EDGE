...
vertex_t *GetDoorVertex(int tx, int ty, int which)
{
	int d_num;
	vertex_t *v;
...
void TryVertex(int x1, int y1, int dx, int dy)
{
	int vx = x1 + ((dx > 0) ? 1 : 0);
	int vy = y1 + ((dy > 0) ? 1 : 0);
...
static void AnalyseMap(void)
{
	...
	sector_matrix = Z_New(short, WLFMAP_W * WLFMAP_H);
	marked_cur_sec = 0;
	...
	door_matrix = Z_New(short, WLFMAP_W * WLFMAP_H);
	num_doors = 0;
