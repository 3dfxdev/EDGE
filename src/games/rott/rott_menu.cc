...
//
//******************************************************************************
extern bool  SetMasterVol(long volume);
bool SkipDeleteMenu = FALSE;//bna++
...
CP_iteminfo MouseBtnItems = { 19, 52, 8, 0, 11, MouseBtnNames, mn_8x8font };
...
CP_MenuNames ControlMMenuNames[] =
{
	...
	"ACTOR OPTIONS",//bna added
	"ENVIRONMENT",//bna added
	"MUSIC VOLUME",
	"SOUND FX VOLUME"

};
CP_iteminfo ControlMItems = { 32, 48 - 8 * 2, 7, 0, 32, ControlMMenuNames, mn_largefont };//bna added
...
CP_itemtype ControlMMenu[] =
{
	...
	{1, "actt\0", 'A', (menuptr)CP_ACTOROPTIONS},//bna added
	{1, "envt\0", 'E', (menuptr)CP_ENVIRONMENT},//bna added
...
CP_iteminfo ExtOptionsItems = { 20, MENU_Y + (8 * 4), 4, 0, 43, ExtOptionsNames, mn_largefont };
...
CP_iteminfo ExtBotsItems = { 20, MENU_Y, 9, 0, 43, ExtBotsNames, mn_largefont };

CP_itemtype ExtBotsMenu[] =
{
	{1, "autoadj\0", 'A', NULL},
	{1, "lightdim\0",'L', NULL},
	{1, "bobbin\0",  'B', NULL},
...
static char SaveName[14] = "rottgam?.rot\0";
...
void DrawStoredGame(byte * pic, int episode, int area, int version);//bna++
...
void SetUpControlPanel(void)
{
	...
	savedscreen = (byte *)SafeMalloc(16000 * 8);
	...
	if (iGLOBAL_SCREENWIDTH == 800) {
		...
		for (i = 0; i < Xres; i += 5) {
			...
			for (j = 0; j < (Yres / 6.00); j++, s++, b += (iGLOBAL_SCREENWIDTH << 1) * 2)
				*s = *b;
		}
	...
	if (iGLOBAL_SCREENWIDTH == 1024) {
		...
		for (i = 0; i < Xres; i += 6) {
			b = (byte *)bufferofs + i;//schrink screen to 1/2 size
			for (j = 0; j < (Yres / 7.68); j++, s++, b += (iGLOBAL_SCREENWIDTH << 1) * 3)
				*s = *b;
		}
	}
	ScanForSavedGames();
...
void AllocateSavedScreenPtr(void)
{
	...
	savedscreen = (byte *)SafeMalloc(16000);
	inmenu = TRUE;
	numdone = 0;
}
...
void ControlPanel(byte scancode) {
	...
	//stops here when user pushed Escape
	//
	//   scancode=scancode;sc_Pause_BNA
		...
		if ((MenuNum == 4) || (MenuNum == 6))
		{
			if (Keyboard[sc_Delete] && SaveGamesAvail[handlewhich])
			{
				if (CP_DisplayMsg("Delete saved game?\nAre you sure?", 12) == TRUE)
				{
					...
					_itoa(handlewhich, &loadname[7], 16);
					loadname[8] = '.';
...
void CP_DrawSelectedGame(int w)
{
	...
	if (SaveGamesAvail[w])
	{
		...
		DrawStoredGame(&game.picture[0], game.episode, game.area, game.picture[0]);
	}
	else
...
void DrawStoredGame(byte * pic, int episode, int area, int version)
{
	char str[3];
	int level;
	byte *shape;
	//zxcv
	shape = (byte *)W_CacheLumpNum(W_GetNumForName("newfnt1"), PU_CACHE, Cvt_font_t, 1);
	...
	//bna section start
	EraseMenuBufRegion(78, 121, 85, 7);
	if ((version > 0) && (version <= 4)) {
		// use game.picture[0] to store info about winrott version
		if (version == 1) { DrawMenuBufPropString(78, 121, "SHAREWARE"); }
		if (version == 2) { DrawMenuBufPropString(78, 121, "SUPERCD"); }
		if (version == 3) { DrawMenuBufPropString(78, 121, "SITELICENSE"); }
		if (version == 4) { DrawMenuBufPropString(78, 121, "REGISTERED"); }
		pic[0] = 0;
	}
	//bna section end
...
int DoLoad(int which)
{
	...
	if ((which >= 0) && SaveGamesAvail[which])
	{
		...
		{
			if (SkipDeleteMenu == TRUE) { goto skipdel; }//bna++
...
void QuickSaveGame(void)
{
	...
	_itoa(quicksaveslot, &loadname[7], 16);
	loadname[8] = '.';
...
int CP_SaveGame(void)
{
	...
	do
	{
		...
		if (which >= 0)
		{
			...
			DrawStoredGame(savedscreen, gamestate.episode, gamestate.mapon, -1);
...
#ifdef DC
			{
				...
				_dos_getdate(date);
				sprintf(input, "E%dA%d %04d/%02d/%02d", gamestate.episode, gamestate.mapon + 1, date->year, date->month, date->day);
...
#endif
			{
				...
				game.picture[0] = 0;
#if ( SHAREWARE == 1 )
				game.picture[0] = 1;//ROTT_SHAREWARE;
#elif ( SUPERROTT == 1 )
				game.picture[0] = 2;//ROTT_SUPERCD;
#elif ( SITELICENSE == 1 )
				game.picture[0] = 3;//ROTT_SITELICENSE;
#else
				game.picture[0] = 4;//ROTT_REGISTERED;
#endif
				// bna section end
...
byte * IN_GetScanName(ScanCode scan)
{
	...
	for (s = ExtScanCodes, p = (byte **)&ExtScanNames[0]; *s; p++, s++)
		if (*s == scan)
			return (*p);
...
void MUSIC_SetVolume(int volume)
{
	...
	//GetSetVolumeMidi (volume, 1); // 0 = get  1 = set bna
	SetMasterVol(MUvolume);
}
...
// 2.15 ext menu includes
//------------------------------------------------------------------------------------
//bna added

CP_MenuNames ExtACTOROPTIONSNames[] = {
	"ACTORS IN NETGAME","RESPAWN ACTORS","PLAY SUPERHARD","EXT MONK ATTACK","BOTS","SELECT BOTS"
};
CP_iteminfo ExtACTOROPTIONSItems = { 20, MENU_Y + (8 * 2), 6, 0, 43, ExtACTOROPTIONSNames, mn_largefont };
CP_itemtype ExtACTOROPTIONSMenu[] = {
	{2, "", 'A', NULL},
	{1, "", 'R', NULL},
	{1, "", 'P', NULL},
	{1, "", 'E', NULL},
	{1, "", 'B', NULL},
	{1, "", 'S', (menuptr)CP_SelectBots}
};
...
void CP_ACTOROPTIONS(void)
{
	...
	DrawACTOROPTIONSMenu();

	do {
		which = HandleMenu(&ExtACTOROPTIONSItems, &ExtACTOROPTIONSMenu[0], NULL);

		if (lstKey != 27) {//dont use if lastscan = escapekey = 27
			switch (which) {
			case 0: useactorsinnet ^= 1; DrawACTOROPTIONSButtons(); break;
			case 1: useRespawn ^= 1; DrawACTOROPTIONSButtons(); break;
			case 2: useSuperhard ^= 1; DrawACTOROPTIONSButtons(); break;
			case 3: useExtMonk ^= 1; DrawACTOROPTIONSButtons(); break;
			case 4: usebots ^= 1; DrawACTOROPTIONSButtons(); break;
			}
		}
...
{
	int i, on, button_on, button_off;

	button_on = W_GetNumForName("snd_on");
	button_off = W_GetNumForName("snd_off");

	for (i = 0; i < ExtACTOROPTIONSItems.amount - 1; i++)
		if (ExtACTOROPTIONSMenu[i].active != CP_Active3) {
			on = 0;
			switch (i) {
			case 0: if (useactorsinnet == 1) on = 1; break;
			case 1: if (useRespawn == 1) on = 1; break;
			case 2: if (useSuperhard == 1) on = 1; break;
			case 3: if (useExtMonk == 1) on = 1; break;
			case 4: if (usebots == 1) on = 1; break;
			}
			if (on)
				DrawMenuBufItem(20 + 22, ExtACTOROPTIONSItems.y + i * 14 - 1, button_on);
			else
				DrawMenuBufItem(20 + 22, ExtACTOROPTIONSItems.y + i * 14 - 1, button_off);
		}
}
...
void DrawACTOROPTIONSMenu(void)
{
	...
	SetAlternateMenuBuf();
	ClearMenuBuf();
	SetMenuTitle("Extended Actor Options");

	MN_GetCursorLocation(&ExtACTOROPTIONSItems, &ExtACTOROPTIONSMenu[0]);
	DrawMenu(&ExtACTOROPTIONSItems, &ExtACTOROPTIONSMenu[0]);
	DrawACTOROPTIONSButtons();
	DisplayInfo(0);
...
/////////////////////////////CP_ENVIRONMENT///////////////////////////////////////
CP_MenuNames ExtENVIRONMENTNames[] = {
	"WIND","RAIN","BASEMARKER"
};
CP_iteminfo ExtENVIRONMENTItems = { 20, MENU_Y + (8 * 5), 3, 0, 43, ExtENVIRONMENTNames, mn_largefont };
CP_itemtype ExtENVIRONMENTMenu[] = {
	{1, "", 'W', NULL},
#if (USERAINCODE==1)
	{1, "", 'R', NULL},
#else
...
void CP_ENVIRONMENT(void)
{
	...
	do {
		which = HandleMenu(&ExtENVIRONMENTItems, &ExtENVIRONMENTMenu[0], NULL);
		switch (which) {
		case 0: useWind ^= 1; DrawENVIRONMENTButtons(); break;
		case 1: useRain ^= 1; DrawENVIRONMENTButtons(); break;
		case 2: UseBaseMarker ^= 1; DrawENVIRONMENTButtons(); break;
		}
...
void DrawENVIRONMENTButtons(void)
{
	...
	for (i = 0; i < ExtENVIRONMENTItems.amount; i++)
		if (ExtENVIRONMENTMenu[i].active != CP_Active3) {
			on = 0;
			switch (i) {
			case 0: if (useWind == 1) on = 1; break;
			case 1: if (useRain == 1) on = 1; break;
			case 2: if (UseBaseMarker == 1) on = 1; break;
			}

			if (on)
				DrawMenuBufItem(20 + 22, ExtENVIRONMENTItems.y + i * 14 - 1, button_on);
			else
				DrawMenuBufItem(20 + 22, ExtENVIRONMENTItems.y + i * 14 - 1, button_off);
		}
}
...
void DrawENVIRONMENTMenu(void)
{
	...
	SetAlternateMenuBuf();
	ClearMenuBuf();
	SetMenuTitle("Environment Options");

	MN_GetCursorLocation(&ExtENVIRONMENTItems, &ExtENVIRONMENTMenu[0]);
	DrawMenu(&ExtENVIRONMENTItems, &ExtENVIRONMENTMenu[0]);
...
void CP_SelectBots(void)
{
	...
	do
	{
		which = HandleMenu(&ExtBotsItems, &ExtBotsMenu[0], NULL);
		switch (which)
		{
		case 0: botsdata[0].usebot ^= 1; CP_BotsMenu(); break;
		case 1: botsdata[1].usebot ^= 1; CP_BotsMenu(); break;
		case 2: botsdata[2].usebot ^= 1; CP_BotsMenu(); break;
		case 3: botsdata[3].usebot ^= 1; CP_BotsMenu(); break;
		case 4: botsdata[4].usebot ^= 1; CP_BotsMenu(); break;
		case 5: botsdata[5].usebot ^= 1; CP_BotsMenu(); break;
		case 6: botsdata[6].usebot ^= 1; CP_BotsMenu(); break;
		case 7: botsdata[7].usebot ^= 1; CP_BotsMenu(); break;
		case 8: botsdata[8].usebot ^= 1; CP_BotsMenu(); break;
		}
	} while (which >= 0);
	DrawACTOROPTIONSMenu();
}

void CP_BotsMenu(void)//void CP_DetailMenu (void)
{
	//      int which;
	int i,
		on;
	...
	for (i = 0; i < ExtBotsItems.amount; i++)
		if (ExtBotsMenu[i].active != CP_Active3)
		{
			//
			// DRAW SELECTED/NOT SELECTED GRAPHIC BUTTONS
			//
			...
			switch (i)
			{
			case 0: if (botsdata[0].usebot == 1) on = 1; break;
			case 1: if (botsdata[1].usebot == 1) on = 1; break;
			case 2: if (botsdata[2].usebot == 1) on = 1; break;
			case 3: if (botsdata[3].usebot == 1) on = 1; break;
			case 4: if (botsdata[4].usebot == 1) on = 1; break;
			case 5: if (botsdata[5].usebot == 1) on = 1; break;
			case 6: if (botsdata[6].usebot == 1) on = 1; break;
			case 7: if (botsdata[7].usebot == 1) on = 1; break;
			case 8: if (botsdata[8].usebot == 1) on = 1; break;
			}

			if (on)
				DrawMenuBufItem(20 + 22, ExtBotsItems.y + i * 14 - 1, button_on);
			else
				DrawMenuBufItem(20 + 22, ExtBotsItems.y + i * 14 - 1, button_off);
		}
	/*
	do
	{
	...
	case 0:
	DetailLevel = 0;
	MN_DrawButtons (&ExtBotsItems, &ExtBotsMenu[0], DetailLevel, OptionNums);
	break;
...
void DrawBotsMenu(void)//void DrawDetailMenu (void)
{
	MenuNum = 1;

	SetAlternateMenuBuf();
	ClearMenuBuf();
	SetMenuTitle("Select Bots");

	//MN_DrawButtons (&ExtBotsItems, &ExtBotsMenu[0], DetailLevel, OptionNums);
	CP_BotsMenu();
	MN_GetCursorLocation(&ExtBotsItems, &ExtBotsMenu[0]);
	DrawMenu(&ExtBotsItems, &ExtBotsMenu[0]);
	DisplayInfo(0);
	FlipMenuBuf();
...

{
	...
	default:
		_itoa(options->DangerDamage, text, 10);
		string = text;
	}
	...
	{
		_itoa(options->TimeLimit, text, 10);
		string = text;
	}
	...
	{
		_itoa(options->HitPoints, text, 10);
		string = text;
	}
	ShowBattleOption(inmenu, PosX, PosY, 1, 5, "Hit Points", string);

	_itoa(options->RespawnTime, text, 10);
...
void SS_SetupSoundBlaster(void);
...
CP_iteminfo SBSetupItems =
{
	MENU_X - 11, MENU_Y + 76, 2, STARTITEM, 19, SBSetupNames, mn_largefont
};
...
void SS_DrawSBSetupMenu(void)
{
	...
	{
		_itoa(SBSettings.Address, num, 16);
		strcat(text, num);
	}
	else
	{
	...
	{
		_itoa(SBSettings.Interrupt, num, 10);
		strcat(text, num);
	}
	else
	{
