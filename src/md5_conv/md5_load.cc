...
#define md5_parse_error I_Error
...
MD5model * md5_load(char *md5text) 
{
	...
	if (try_parse()) 
	{
		...
		{
			md5_parse_error("MD5: %m cannot parse expected numJoints (bad export?)");
		}
...
{
	//TODO should replace with hash lookup (probably epi::md5_hash)
	int i;
