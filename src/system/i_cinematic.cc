...
static void CIN_DecodeSoundStereo22 (cinematic_t *cin, const byte *data){
	...
	//S_RawSamples(cin_soundSamples, cin->chunkHeader.size >> 1, 22050, true);
}
...
cinHandle_t CIN_PlayCinematic (const char *name, int flags)
{
	...
	if (name)
	{
		...
		F->Read(&cin_chunkData, sizeof(ROQ_CHUNK_HEADER_SIZE));
...
void E_PlayMovie(const char *name, int flags)
{
	...
	{
		CIN_Shutdown();
		I_Printf("WARNING: Could not open ROQ: %s", midx);
	...
	while (playing_movie)
	{
		...
		mdata = CIN_UpdateCinematic(midx, curr_ms);
