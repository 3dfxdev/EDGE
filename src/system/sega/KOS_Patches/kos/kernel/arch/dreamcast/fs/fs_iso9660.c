...
Switch to simple read-ahead cache by Joe Fenton.
...
/********************************************************************************/
/* Low-level block cacheing routines. This implements a simple read-ahead
   cache based on lines of sectors */

/* Holds the sector number and a pointer to the data. */
typedef struct {
...
#define LINE_SHIFT  2
#define CACHE_SHIFT 2
#define NUM_SECT_PER_LINE   (1 << LINE_SHIFT)
#define NUM_LINES_PER_CACHE (1 << CACHE_SHIFT)
#define NUM_CACHE_BLOCKS (NUM_SECT_PER_LINE * NUM_LINES_PER_CACHE)
static cache_block_t icache[NUM_CACHE_BLOCKS];     /* inode cache */
static cache_block_t dcache[NUM_CACHE_BLOCKS];     /* data cache */

/* This points to the actual data storage for the cache */
static uint8 *icache_data;
static uint8 *dcache_data;
...
static int bread_cache(cache_block_t *cache, uint32 sector) {
    ...
    /* Check if sector in cache line */
    i = sector & (NUM_SECT_PER_LINE - 1);
    j = (sector >> LINE_SHIFT) & (NUM_LINES_PER_CACHE - 1);
    rv = i + j * NUM_SECT_PER_LINE;
    if (cache[rv].sector == (int)sector)
    {
        mutex_unlock(&cache_mutex);
        return rv;
    }
    ...
    /* Load the cache line */
    k = cdrom_read_sectors(cache[j << LINE_SHIFT].data, (sector & ~(NUM_SECT_PER_LINE-1)) + 150, NUM_SECT_PER_LINE);
    if(k < 0) {
        //dbglog(DBG_ERROR, "fs_iso9660: can't read_sectors for %d: %d\n",
        //  sector+150, k);
        if(k == ERR_DISC_CHG || k == ERR_NO_DISC) {
            init_percd();
        }

        cache[(j << LINE_SHIFT) + 0].sector = -1;
        cache[(j << LINE_SHIFT) + 1].sector = -1;
        cache[(j << LINE_SHIFT) + 2].sector = -1;
        cache[(j << LINE_SHIFT) + 3].sector = -1;
        rv = -1;
    } else {
        cache[(j << LINE_SHIFT) + 0].sector = (sector & ~3) + 0;
        cache[(j << LINE_SHIFT) + 1].sector = (sector & ~3) + 1;
        cache[(j << LINE_SHIFT) + 2].sector = (sector & ~3) + 2;
        cache[(j << LINE_SHIFT) + 3].sector = (sector & ~3) + 3;
    }
...
int fs_iso9660_init() {
    ...
    /* Allocate cache block storage and init cache */
    icache_data = malloc(NUM_CACHE_BLOCKS * 2048);
    dcache_data = malloc(NUM_CACHE_BLOCKS * 2048);
    for(i = 0; i < NUM_CACHE_BLOCKS; i++) {
        icache[i].data = icache_data + i * 2048;
        icache[i].sector = -1;
        dcache[i].data = dcache_data + i * 2048;
        dcache[i].sector = -1;
    }
