...
static bool              videoFinished;

// all audio frames processed
static bool              audioFinished;
...
static bool I_AVPopPacketFromQueue(avPacketQueue_t *packetQueue, AVPacket **packet)
{
    bool result = true;
...
static uint8_t *I_AVPopAudioFromQueue(const int size, double *timestamp, bool *filled)
{
...
static bool I_AVVideoClockBehind(void)
{
...
static bool I_AVSetupCodecContext(AVCodecContext **context, AVCodec **codec, int index)
{
    if(index < 0)
    {
...
static bool I_AVLoadVideo(const char *filename)
{
    int i;
    int videoSize;
    int audioSize;
...
static bool I_AVCapFrameRate(void)
{
