...
static void RoQReset() {
	...
	FS_FCloseFile( cinTable[currentHandle].fp );
	cinTable[currentHandle].fp = 0;
	// dhm - end

	FS_FOpenFileRead( cinTable[currentHandle].fileName, &cinTable[currentHandle].fp, qtrue );

    ::FS_Read(cin.file, 16, cinTable[currentHandle].fp);
...
/* #if defined RTCW_SP
			if ( cinTable[currentHandle].numQuads == -1 ) {
				S_Update();
				I_Printf( "S_Update: Setting rawend to %i\n", s_soundtime );
...
//
	...
	if ( cinTable[currentHandle].RoQFrameSize > 65536 || cinTable[currentHandle].roq_id == 0x1084 ) {
		I_Printf( "roq_size>65536||roq_id==0x1084\n" );
		cinTable[currentHandle].status = FMV_EOF;
		if ( cinTable[currentHandle].looping ) {
			RoQReset();
		}
...
//	r = Sys_StreamedRead( cin.file, cinTable[currentHandle].RoQFrameSize+8, 1, cinTable[currentHandle].fp );
	cinTable[currentHandle].RoQPlayed   += cinTable[currentHandle].RoQFrameSize + 8;
}
...
static void RoQShutdown( void ) {
	...
	I_Printf( "finished cinematic\n" );
	cinTable[currentHandle].status = FMV_IDLE;

	if ( cinTable[currentHandle].fp ) {
		FS_FCloseFile( cinTable[currentHandle].fp );
		cinTable[currentHandle].fp = 0;
	}
...
int CIN_PlayCinematic( const char *arg, int x, int y, int w, int h, int systemBits ) {
	...
	if ( strstr( arg, "/" ) == NULL && strstr( arg, "\\" ) == NULL ) {
		I_Debugf( name, sizeof( name ), "video/%s", arg );
	} else {
		I_Debugf( name, sizeof( name ), "%s", arg );
	}
	...
	cinTable[currentHandle].ROQSize = FS_FOpenFileRead( cinTable[currentHandle].fileName, &cinTable[currentHandle].fp, qtrue );

	if ( cinTable[currentHandle].ROQSize <= 0 ) {
		I_Printf( "play(%s), ROQSize<=0\n", arg );
		cinTable[currentHandle].fileName[0] = 0;
		return -1;
	}
	...
	if ( RoQID == 0x1084 ) {
		RoQ_init();
//		FS_Read (cin.file, cinTable[currentHandle].RoQFrameSize+8, cinTable[currentHandle].fp);

		cinTable[currentHandle].status = FMV_PLAY;
		I_Printf( "trFMV::play(), playing %s\n", arg );
 ...
 */
	...
	I_Printf( "trFMV::play(), invalid RoQ ID\n" );
