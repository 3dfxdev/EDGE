...
/*
*
* ParseAVI/ROQ Video - Copyright 2015-2016 by Chilly Willy <ChillyWillyGuru@gmail.com>
*
* based on
...
* This code falls under the BSD license.
  ...
  2018: Coraline: Cleaned up for EDGE 2.1.0
*
*/
...
static int hex_dump_chunk(epi::file_c *in, int chunk_len)
{
...
static int read_avi_header(epi::file_c *in)
{
	int i;
	...
	/* read header */
	in->Read(&header, sizeof(avi_header_t));

	u32_t offset = f_tell(in);
...
static int read_stream_header(epi::file_c *in)
{
	u32_t offset = EPI_LE_S32(in);
...
static int read_stream_header_auds(epi::file_c *in)
{
...
static int read_stream_format(epi::file_c *in)
{
	int t, r, g, b;
	long offset = f_tell(in);
	...
	if (stream_format.colors_important != 0)
	{
		...
		{
			// the struct we need to get these from is stream_format.palette, I think
			//b = fgetc(in);
			//g = fgetc(in);
			//r = fgetc(in);
...
static int read_stream_format_auds(epi::file_c *in)
{
	long offset = f_tell(in);
...
static int read_stream_header_txts(epi::file_c *in)
{
	long offset = f_tell(in);
...
static int parse_hdrl_list(epi::file_c *in)
{
	char chunk_id[5];
	int chunk_size;
	char chunk_type[5];
	...
	{
		in->Seek(end_of_chunk, epi::file_c::SEEKPOINT_START);
	...
	while (f_tell(in) < end_of_chunk)
	{
		...
		if (strcasecmp("strh", chunk_type) == 0)
		{
			...
			in->Seek(marker, epi::file_c::SEEKPOINT_START);
		...
		in->Seek(next_chunk, epi::file_c::SEEKPOINT_START);
...
static int parse_hdrl(epi::file_c *in, unsigned int size)
{
	char chunk_id[5];
	int chunk_size;
	int end_of_chunk;
...
//
int parse_movi(epi::file_c *in, int *length, bool retv, GLuint *txr, bool reta, short *buf, int samples, float ascale)
{
	char chunk_id[5];
	int chunk_size;
	int end_of_subchunk;
	...
	if (*length == -1)
	{
		in->Seek(movi_pos, epi::file_c::SEEKPOINT_START);
		*length = movi_size;
...
int parse_avi(epi::file_c *in)
{
	...
	chunk_size = EPI_LE_U16(in);

	in->Read(chunk_type, sizeof(in));

	in->Seek(offset, epi::file_c::SEEKPOINT_START);
#ifdef FMV_DEBUG
	I_Error("RIFF Chunk (id=%s, size=%d, type=%s, offset=0x%lx)\n", chunk_id, chunk_size, chunk_type, offset);
	//I_Printf("{\n");
#endif
	...
	while (f_tell(in) < end_of_chunk)
	{
		...
		in->Seek(end_of_subchunk, epi::file_c::SEEKPOINT_START);
