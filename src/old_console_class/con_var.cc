...
void CON_CvarSet(const char *name, const char *flags, const char *value) 
{
    ///cvar_t *var;
    bool changed;

    cvar_link_t *L = CON_FindVar(name); ///cast L into cvar_link_t, name
...
void CON_CvarSetValue(char *name, float value)
{
    char val[32];

    sprintf(val, "%f",value);
    CON_CvarSet(name, "%s", val);
}
