...
static void resetDefaults(uint32_t sample_rate, uint8_t is_rsid, 
	...
	WebSID::resetAll(sample_rate, clock_rate, is_rsid, is_compatible);
...
#define SYNTH_NORMAL(synth_buffer, synth_trace_bufs, scale, i) \
	if (WebSID::isAudible()) { \
		WebSID::synthSample(synth_buffer, synth_trace_bufs, &scale, i); \
	} else { \
		synth_buffer[(i<<1)]= 0; \
	}
...
void runEmulation(uint8_t is_simple_sid_mode, int16_t* synth_buffer, 
	...
	double n= WebSID::getCyclesPerSample();
	...
	double scale = WebSID::getScale();	// avoid recalc within loop
	...
	if (is_simple_sid_mode) {
		...
		for (int i= 0; i<samples_per_call; i++) {
			...
			WebSID::synthSampleStripped(synth_buffer, synth_trace_bufs, &scale, i);
		}
	}
}
